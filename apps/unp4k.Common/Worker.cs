...
public static class Worker
{
    ...
    private static async Task ExtractCommon(Enviroment env)
    {
        ...
        Logger.NewLine(2);
        if (P4K is not null && P4K.EntryCount is not 0) // This will never be null but it makes the analyser happy.
        {
            ...
            void print(BlockingCollection<ZipEntry> queue, Stopwatch fileTime) // TODO: Add ability to send errors through to this and flag the square as red.
            {
                if (P4K is not null) // This will never be null but it makes the analyser happy.
                {
                    foreach (ZipEntry entry in queue.GetConsumingEnumerable())
                    {
                        ...
                        if (env is Enviroment.Terminal) Logger.SetTitle($"unp4k: {percentage}%");
                    }
                }
            }
        }
        ...
        if (env is Enviroment.Terminal)
        {
            // This will never be null but it makes the analyser happy.
